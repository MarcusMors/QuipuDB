...
void parsing(char *data)
{
  ...
  case action::Update: {
    ...
    // Guardamos en el char "U" + tamaño1 + nombre1 + tamaño2 + nombre2 + tamaño3 + relacion + tamaño4 + newName1 +
    // tamaño5 + newName2 + tamaño6 + newRelation
    string new_char = "U" + size1_str + name1 + size2_str + name2 + size3_str + relation + size4_str + newName1
                      + size5_str + newName2 + size6_str + newRelation;
...
int main()
{
  // connectToSocket(serverSock, server_addr, app_base_port);
  client = Client(app_base_port);
  ...
  while (true) {
    ...
    printf("\n$:");
    cin.getline(send_data, packet_size);
    ...
    if (!client.clientSend(send_data)) {
      cout << "No se pudo enviar los datos\n";
      continue;
    }

    // Si se envio una solicitud de lectura, esperar respuesta
    if (send_data[0] == action::Read) {// Lectura espera por respuesta
      cout << "ESPERANDO DATA: \n";
      msg = client.receive();
      cout << msg.substr(5, msg.size() - 5) << "\n";
    }
  }
}
