...
// KeepAliveVector
bool onlineServers[subservers_num];
...
struct sockaddr_in subServers[subservers_num];
struct sockaddr_in keepAlive[subservers_num];
...
{
  int mod = msg[3] % subservers_num;// Seleccionar el subserver
  sendMsgToSubServer(mod, msg);
  sendMsgToSubServer((mod + 1) % subservers_num, msg);
}
...
string recursiveRead(string base, string query, int maxRecursive, int deep = 0)
{
  ...
  bool alive;
  int mod = query[0] % subservers_num;
...
void waitSubServers()
{
  ...
  for (int i = 0; i < subservers_num; ++i) {
    ...
    if (answ == subServerWave)
      subServers[i] = mainServer_Sub.getLastReadClient();
    else
      --i;

    // Get KeepAlive's
    answ = mainServer_KeepAlive.serverRecive();
    if (answ == keepAliveData)
      keepAlive[i] = mainServer_KeepAlive.getLastReadClient();
    else
      --i;
  }
}
...
void listenClients()
{
  ...
  while (true) {
    // recived_data = receive_msg(client_addr, addr_lenClient, sockClient);
    recived_data = mainServer_Client.serverRecive();


    if (recived_data[0] != 'R')
      thread(simpleRedirect, recived_data).detach();
    else {
...
int main()
{
  ...
  // Inicializar Sockets
  mainServer_Client = Server(app_base_port);
  mainServer_Sub = Server(app_base_port + 1);
  mainServer_KeepAlive = Server(app_base_port + 2);

  // Need to know all subservers ports before keep alive and query's
  waitSubServers();
  printlnr("All subservers ", Style::yellow, "READY");
